...
contract AMM {
    ...
    function getPrice(uint256 _amount) public view returns (uint256 amount) {

        // Returns Token 1 (DAPP) in terms of Token 2 (USD)
        return (_amount * token2Balance) / token1Balance;
    }

    function getReversePrice(uint256 _amount) public view returns (uint256 amount) {

        // Returns Token 2 (USD) in terms of Token 1 (DAPP)
        return (_amount * token1Balance) / token2Balance;
    }
